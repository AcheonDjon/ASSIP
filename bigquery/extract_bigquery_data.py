...
and lower(repo.name) LIKE lower(@company_name))
...
(CASE WHEN type = 'ReleaseEvent' THEN payload ELSE NULL END) AS release_payload
FROM (SELECT * FROM `githubarchive.month.*` WHERE _TABLE_SUFFIX BETWEEN @StartYear AND @EndYear) t1 WHERE lower(t1.repo.name) LIKE lower(@company_name)  
...
#'aeonix','staratlasmeta', 'quark-project','casinolandnetwork',  'KMPARDS', 'fr8network', 'Beaboutyourlife', 'impactMarket', 'linear-protocol','MindCoinTeam','niftyx','PanIndustrial-Org','PrimLabs', 'RubiconDeFi', 'Solanascan','ImpeccableHQ','TrSoft-Inc', 'wannaswap']
